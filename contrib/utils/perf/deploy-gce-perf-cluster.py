...
def deploy_cluster(args):
  ...
  """
    ...
    if not agents_dict or len(agents_dict) < number_of_nodes:
        raise Exception(
            "Cannot bring up enough nodes. Requested {0}, but got {1}. Probably not enough resources!".format(
                number_of_nodes, len(agents_dict)))
    ...
    for (hostname, ip) in sorted_agents:
        ...
        print("==========================")
        print("Working on VM {0} that will contain hosts {1} - {2}".format(hostname, start_num,
                                                                           start_num + num_agents_on_this_host - 1))

        # The agent multiplier config will be different on each VM.

        cmd_generate_multiplier_conf = "mkdir -p /etc/ambari-agent/conf/ ; printf \"start={0}\\nnum={1}\\nprefix={2}\" > /etc/ambari-agent/conf/agent-multiplier.conf".format(
            start_num, num_agents_on_this_host, args.agent_prefix)
...
def create_vms(args, number_of_nodes):
  ...
  """
    print(
        "Creating server VM {0}-server-{1} with xxlarge nodes on centos7...".format(cluster_prefix,
                                                                                    args.cluster_suffix))
    execute_command(args, args.controller,
                    "/opt/gce-utils/gce up {0}-server-{1} 1 --centos7 --xxlarge --ex --disk-xxlarge --ssd".format(
                        cluster_prefix, args.cluster_suffix),
                    "Failed to create server, probably not enough resources!", "-tt")
    time.sleep(10)

    # trying to create cluster with needed params
    print(
        "Creating agent VMs {0}-agent-{1} with {2} xlarge nodes on centos7...".format(cluster_prefix,
                                                                                      args.cluster_suffix,
                                                                                      str(number_of_nodes)))
    execute_command(args, args.controller,
                    "/opt/gce-utils/gce up {0}-agent-{1} {2} --centos7 --xlarge --ex --disk-xlarge".format(
                        cluster_prefix, args.cluster_suffix, str(number_of_nodes)),
...
def prepare_server(args, hostname, ip):
    ...
    print(
        "Executing remote ssh command (set correct permissions and start executing server.sh in separate process) on {0}...".format(
            hostname))
...
def prepare_agent(args, hostname, ip, cmd_generate_multiplier_conf):
    ...
    execute_command(args, ip, cmd_generate_multiplier_conf,
                    "Failed to generate agent-multiplier.conf on host {0}".format(hostname))

    print(
        "Executing remote ssh command (set correct permissions and start executing agent.sh in separate process) on {0}...".format(
            hostname))
  ...
  """
    ...
    contents = "#!/bin/bash\n" + \
               ...
               "sed -i -e 's/agent]/agent]\\nhostname_script={0}\\npublic_hostname_script={1}\\n/1' /etc/ambari-agent/conf/ambari-agent.ini\n".format(
                   hostname_script, public_hostname_script) + \
